...
class QNetwork(nn.Module):
    def __init__(self, observation_space_shape, action_space_dim):
        ...
        self.fc1 = nn.Linear(num_agents + np.array(observation_space_shape).prod(), hidden_size)
...
eg_agent = agents[0]
observation_space_shape = tuple(shape * num_agents for shape in observation_spaces[eg_agent].shape) if args.global_obs else observation_spaces[eg_agent].shape
rb = {}
for agent in agents:
...
for global_step in range(args.total_timesteps):
    ...
    for agent in agents:
        if random.random() < epsilon:
            ...
            # print(obses[agent].shape)
            # print(obses[agent].reshape((1,)+obses[agent].shape).shape)
